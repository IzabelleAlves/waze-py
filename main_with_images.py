...
class WazeApp:
    ...
    def draw_ui(self):
        ...
        if self.path is not None and self.start_pos and self.end_pos:
            ...
            info_text = f"Rota: {path_length} celulas | A({self.start_pos[0]},{self.start_pos[1]}) -> B({self.end_pos[0]},{self.end_pos[1]})"
                ...
                # Botões de ação
                    ...
                    if self.start_pos and self.end_pos:
                        ...
                        if self.path is None:
                            ...
                            self.add_message(f"Rota encontrada: {len(self.path)} celulas", "success")
                    else:
                            ...
                            if self.city.is_walkable(x, y):
                                if self.start_pos is None:
                                    ...
                                    if self.path is None:
                                        ...
                                        self.add_message(f"Rota calculada: {len(self.path)} celulas", "success", 3000)
                                else:
                                    # Reinicia
                                    self.start_pos = cell
                                    self.end_pos = None
